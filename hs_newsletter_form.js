...
$('#slidebox input[type=submit]').on('click', function(e) {
  ...
  if (params && params.length > 1 && submitToggle ) {
    var request = $.ajax({
      ...
      complete: function(data) {
        setTimeout(function() { $('.sa-cta-container').delay(1500).fadeOut(); }, 3000);
      }
    });
  ...
  function hideContainer() {
    ...
    $('.sa-cta-container').hide();
  }

  function showContainer() {
    $(e.target).prop('disabled', false);
    $('.loader').addClass('hide');
    $('.sa-cta-container').show();
  }

  function showThanks(){
    $('.sa-bot-section').fadeOut();
    $('.sa-mid-section').fadeOut();
    $('.sa-top-section').fadeOut();
